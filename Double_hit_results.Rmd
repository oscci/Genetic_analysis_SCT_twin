...
#Correction made 19th May 2021 - line 518 : some SNPs had been omitted from Appendix 9
...
redcap.dir <-"~/Dropbox/ERCadvanced/project SCT analysis/Data from Redcap/old versions/"
#Use latest version
...
sct.short <- dplyr::select(sct.data,all_of(keepcols)) #modified to 'all_of' - suggestion by R
twin.short <-dplyr::select(twin.data,all_of(keepcols))
#----------------------------------------------------------------------------------
```
...
NB with a total of 3 tests for each of 47 SNPs, there are 141 tests for each of the 3 groups; Bonferroni corrected p-value is .0001. See below for distribution of p-values, none of which was less than the Bonferroni-corrected value.
...
sumtab1a<-t(sumtable[7:11,])  #previously omitted in error
...
w<-which(is.na(sumtab1))
sumtab1[w]<-0
trytab<-data.frame(sumtab1)


trytab <-cbind(row.names(trytab),trytab)
colnames(trytab)<-c('_',1:6)
flextable::regulartable(trytab)
...
short.data <- short.data[,-c(1:48)]
short.data <- short.data[,c(1,33:35,41,10,37,38,3:9,11:25,27:31)]
write.csv(short.data,'osf_pheno_sct_twin.csv')
...
simvars <- c(85,92,2:48,58,88:89) #trisomy, bias, all SNPS, then 3 phenos
sct.all <-all.data[1:130,simvars]
syn.all <-syn(sct.all) #NB this creates a list; we need $syn part
syncompare<-compare.synds(syn.all,sct.all) #compare function only works prior to unlisting
...
sctcovs <- cov(sct.all)
sctcovs <- rbind(sctcovs,colMeans(sct.all))
row.names(sctcovs)[53]='ColMean'
write.csv(syn.data,'synthpop_sct.csv')
write.csv(sctcovs,'real_covs_sct.csv')
```
```{R simulatetwinA, echo=FALSE}
#create simulated dataset for twinA

simvars <- c(83,84,2:48,58,88:89) #zygosity, all SNPS, then 3 phenos
twin.a <-all.data[,simvars]
twin.a <- filter(twin.a,TwinOrSCT=='Twin1')
twin.a <-twin.a[,-1] #remove twin column
syn.twina <-syn(twin.a) #NB this creates a list; we need $syn part
twinsyncompare <- compare.synds(syn.twina,twin.a) #compare function only works prior to unlisting
twinsyncompare$plots
#This shows the frequencies of different values in original and synthesised data
syn.twina <-syn.twina$syn #just the synthesised data
rsyntwin<-cor(as.matrix(syn.twina))
rtwin<-cor(twin.a)
rdifftwin<-rsyntwin-rtwin #differences between real and synthetised correlations
hist(rdifftwin,breaks=20,main='Difference between real/synthesised dataset: correlations')  


twincovs <- cov(twin.a)
twincovs <- rbind(twincovs,colMeans(twin.a))
row.names(twincovs)[52]='ColMean'
write.csv(syn.twina,'synthpop_twina.csv')
write.csv(twincovs,'real_covs_twina.csv')
```
