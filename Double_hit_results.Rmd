...
mynperm=100 # to test prog use 100 for speed; for final version alter to 5000 (takes several hrs to run)
...
1. Check that strong and consistent effects of individual SNPs are not being missed.
...
sumtable <- data.frame(matrix(0,47,10)) #create table to summarize freqs of SNPs
colnames(sumtable)<-c('SCT0','SCT1','SCT2','twA0','twA1','twA2','twB0','twB1','twB2')
for (ii in 2:48){
  thist <- table(all.data$randomtwininclude,all.data[,ii])
  sumtable[(ii-1),2:4]<-thist[c(1,4,7)]
  sumtable[(ii-1),5:7]<-thist[c(2,5,8)]
  sumtable[(ii-1),8:10]<-thist[c(3,6,9)]
...
```
```{r reducedset}
#2. Try redoing after omitting those with no homozygous rare allele
doreduce=0
if (doreduce==1){
...
```
```{r reducedset2}
#3. Try redoing after omitting those that correlate > .9 with another SNP
doreduce2=0
if (doreduce2==1){
  mynperm=100 
mykeep <-vector()
